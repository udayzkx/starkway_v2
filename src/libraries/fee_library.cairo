...
mod fee_library {
    ...
    fn set_fee_range(token_l1_address: L1Address, fee_range: FeeRange) {
        assert(u256 {low:0, high:0} <= fee_range.max, 'u256{low:0, high:0} > Max value');
    ...
    fn set_fee_segment(token_l1_address: L1Address, tier: u8, fee_segment: FeeSegment) {
        assert(tier >= 1, 'tier cannot be negative');
        assert(u256 {low:0, high:0} <= fee_segment.fee_rate, 'u256{low:0, high:0} > fee rate');
        assert(u256 {low:0, high:0} <= fee_segment.to_amount, 'u256{low:0, high:0} > to amount');
        ...
        if tier - 1 != 0 {
            assert(lower_tier_fee.to_amount < fee_segment.to_amount, 'amount invalid wrt lower tier');
            assert(fee_segment.fee_rate < lower_tier_fee.fee_rate, 'fee invalid wrt lower tier');
        } else {
            assert(fee_segment.to_amount == u256 {low:0, high:0}, 'to_amount of tier1 should be 0');
        }

        let upper_tier_fee = s_fee_segments::read((token_l1_address, tier + 1));
        if max_fee_segment_tier > tier {
            assert(fee_segment.to_amount < upper_tier_fee.to_amount, 'amount invalid wrt upper tier');
