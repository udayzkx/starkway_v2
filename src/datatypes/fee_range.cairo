...
impl StorageAccessFeeRange of StorageAccess<FeeRange> {
    fn read(address_domain: u32, base: StorageBaseAddress) -> SyscallResult<FeeRange> {
        ...
        let min_low = storage_read_syscall(address_domain, min_address_low)?
            .try_into()
            .expect('non u128 value');
        let min_high = storage_read_syscall(address_domain, min_address_high)?
            .try_into()
            .expect('non u128 value');
        ...
        Result::Ok(
            FeeRange {
                is_set: is_set, min: u256 {
                    low: min_low, high: min_high
                    }, max: u256 {
                    low: max_low, high: max_high
                }
            }
        )
    }
