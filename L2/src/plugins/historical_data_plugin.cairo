...
mod HistoricalDataPlugin {
    ...
    use starknet::{
        ContractAddress, contract_address::ContractAddressZeroable, EthAddress, get_caller_address,
        contract_address::Felt252TryIntoContractAddress,
        contract_address::contract_address_try_from_felt252, get_contract_address,
        get_block_timestamp, eth_address::EthAddressZeroable
    };
    ...
    impl HistoricalDataPluginImpl of IHistoricalDataPlugin<ContractState> {
        ...
        fn get_message_info_at_index(
            self: @ContractState, consumer: ContractAddress, message_index: u64
        ) -> MessageBasicInfo {
        ...
        fn handle_starkway_deposit_message(
            ...
            let consumer: ContractAddress = contract_address_try_from_felt252(temp_felt_address)
                .unwrap();

            assert(
                self._resolve_is_allowed_to_write(consumer, l1_sender_address),
                'HDP: Unauthorised write'
            );
            ...
            loop {
                ...
                self
                    .msg_payload_data
                    .write((consumer, msg_count, index), *message_payload.at(index));
                index += 1;
            };
        }
        ...
        fn add_to_allow_list(ref self: ContractState, eth_address: EthAddress) {
            ...
            let consumer = get_caller_address();
            assert(
                !self.whitelisted_address.read((consumer, eth_address)), 'HDP: Already whitelisted'
            );
        ...
        fn remove_from_allow_list(ref self: ContractState, eth_address: EthAddress) {
            let consumer = get_caller_address();
            assert(
                self.whitelisted_address.read((consumer, eth_address)),
                'HDP: Already de-whitelisted'
            );
    ...
    impl HistoricalDataPluginPrivatefunctions of IHistoricalDataPluginPrivatefunctions {
        ...
        /// @return - basic message info (ref: MessageBasicInfo struct) and message payload
        fn _read_message(
            self: @ContractState, consumer: ContractAddress, msg_index: u64
        ) -> (MessageBasicInfo, Array<felt252>) {
            assert(
                msg_index < self.number_of_messages.read(consumer), 'HDP: Msg index out of bounds'
            );
        ...
        fn _resolve_is_allowed_to_write(
            ...
            self.whitelisted_address.read((consumer, writer))
        }
    }
}
